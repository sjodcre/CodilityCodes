...
function solution(A) {
    ...
    let firstHalfSum= A[0];
    // console.log(firstHalfSum)
    let secondHalfSum = sum(A)-firstHalfSum;
    // console.log(secondHalfSum)
    let difference = Math.abs(secondHalfSum-firstHalfSum);
    ...
    for (let p =1; p<A.length-1;p++) {
        firstHalfSum +=A[p];
        secondHalfSum-=A[p];
        currentDifference = Math.abs(secondHalfSum-firstHalfSum);
